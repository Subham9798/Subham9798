<div align="center">

# `Subham9798@github ~ $ whoami`

<img src="avi-ascii.svg" width="700" alt="Subham Kumar Singh — custom ASCII portrait" />

<img src="wordmark.svg" width="700" alt="Subham Kumar Singh — 3D ASCII wordmark" />

**Software Developer · Computer Science · AI**

[![GitHub](https://img.shields.io/badge/GitHub-Subham9798-111111?style=for-the-badge&logo=github)](https://github.com/Subham9798)

</div>

---

### `> about_me`

- 💻 Building software with **Computer Science, Python, C/C++ and AI**
- 🤖 Interested in **Explainable AI** and practical AI-powered applications
- 🌦️ Working on web applications with **AI + weather support**
- 📚 Practicing **DSA and LeetCode** consistently

### `> tech_stack`

`Python` `C` `C++` `OpenCV` `PyTorch` `LaTeX` `GitHub Actions`

### `> github`

<p align="center">
  <a href="https://github.com/Subham9798">
    <img src="https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github" alt="GitHub Profile" />
  </a>
  <a href="https://github.com/Subham9798?tab=repositories">
    <img src="https://img.shields.io/badge/Repositories-View-181717?style=for-the-badge&logo=github" alt="View repositories" />
  </a>
</p>

### `> contributions`

![GitHub Contribution Graph](https://ghchart.rshah.org/Subham9798)

---

<div align="center">

`Subham Kumar Singh`  ·  `Subham9798`

</div>
